'use strict';
(function(){
  const BOOT_VERSION='the-board-app-ready-22.0.0';
  ...
  async function boot(){
    ...
    addStyle(`live-draft-engine-v2.css?v=${v}`,'live-draft-engine');
    ...
    await addScript(`live-draft-engine-v2.js?v=${v}`,'live-draft-engine');window.BoardDraftEngine?.refresh?.();
    window.BoardPresentation?.apply?.();
    document.documentElement.dataset.cockpitReady='true';
    window.dispatchEvent(new CustomEvent('theboard:app-ready',{detail:{version:BOOT_VERSION,core:window.BoardCore.version}}));
  }
