'use strict';
(function(){
  const BOOT_VERSION='the-board-app-ready-22.1.0';
  ...
  async function boot(){
    ...
    document.querySelectorAll('script[src*="front-office-phase1"],script[src*="front-office-polish"],script[src*="front-office-photo-fix"],script[src*="front-office-photo-integrity"],script[src*="front-office-position-colors"],script[src*="front-office-intelligence-v2"],script[src*="command-surface"],script[src*="cockpit-v1"],script[src*="cockpit-v2"],script[src*="cockpit-v3"],script[src*="cockpit-v4"],script[src*="cockpit-v5"],script[src*="cockpit-route-sync"],script[src*="integrated-board"],script[src*="live-draft-engine"],script[src*="state-bridge"],style#phaseOneStyles,link[data-front-office-clean],link[data-front-office-style],script[data-front-office-clean],script[data-front-office-script]').forEach(n=>n.remove());
    ...
    await addScript(`state-bridge-v1.js?v=${v}`,'state-bridge');await addScript(`foundation-controls-v1.js?v=${v}`,'foundation-controls');await addScript(`front-office-clean.js?v=${v}`,'renderer');
    window.renderWarroom=()=>window.CleanFrontOffice?.render?.();window.renderWarroom();
    await addScript(`cockpit-v5-2.js?v=${v}`,'cockpit-v52');window.BoardCockpitV52?.apply?.();await addScript(`cockpit-route-sync-v1.js?v=${v}`,'route-sync');window.BoardRouteSync?.wire?.();await addScript(`integrated-board-v2.js?v=${v}`,'integrated-board');window.IntegratedBoardV2?.render?.();await addScript(`live-draft-engine-v2.js?v=${v}`,'live-draft-engine');window.BoardDraftEngine?.refresh?.();window.BoardPresentation?.apply?.();document.documentElement.dataset.cockpitReady='true';window.dispatchEvent(new CustomEvent('theboard:app-ready',{detail:{version:BOOT_VERSION,core:window.BoardCore.version}}));
  }
