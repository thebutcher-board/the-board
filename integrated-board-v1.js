'use strict';
(function(){
  const VERSION='integrated-board-1.0.0';
  ...
  const ownerAt=i=>{try{return window.draftOrderAt?.(i)||`Team ${(i%teamCount())+1}`}catch{return `Team ${(i%teamCount())+1}`}};
  ...
  function goDraft(){window.__TB_FOUNDATION_CONTROLS__?.goToDraft?.();}
  function render(){
    const board=document.getElementById('board');
    if(!board)return false;
    const active=location.hash==='#board'||board.classList.contains('active');
    if(!active)return false;
    document.body.classList.add('tb-board-active');
    const count=teamCount();
    const teams=Array.from({length:count},(_,i)=>ownerAt(i));
    ...
    const rounds=Math.max(10,Math.ceil(Math.max(drafted().length,count*10)/count));
    const cells=[];
  ...
  const observer=new MutationObserver(()=>{if(location.hash==='#board'&&!document.querySelector('#board .tb-board-app'))schedule()});
  const boot=()=>{observer.observe(document.body,{childList:true,subtree:true});if(location.hash==='#board')schedule();window.IntegratedBoardV1={render,version:VERSION}};
