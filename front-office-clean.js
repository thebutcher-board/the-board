...
(function(){
  const PHOTO_IDS={
    ...
    'brock purdy':'7523','jared goff':'5857','matthew stafford':'421','jaxson dart':'12507','patrick mahomes':'4046','patrick mahomesq':'4046',
  ...
  function playerPhoto(name,cls=''){
    const id=PHOTO_IDS[clean(name)];
    const fallback=esc(initials(name));
    if(!id)return `<span class="tb-photo ${cls} is-fallback"><b>${fallback}</b></span>`;
  ...
  function hydratePhotos(root){
    root.querySelectorAll('.tb-photo:not(.is-fallback)').forEach(node=>{
      ...
      const name=node.dataset.photoName||''; const id=PHOTO_IDS[clean(name)]; let triedThumb=false;
      img.onerror=()=>{
        if(!triedThumb&&id){triedThumb=true;img.src=`https://sleepercdn.com/content/nfl/players/thumb/${id}.jpg`;return;}
        img.remove();node.classList.add('is-fallback');
      };
    });
  }
  function itemFor(p){return engine().find(x=>x.player?.name===p?.name)||{player:p,factors:{need:72,scarcity:58}}}
  function metrics(item){
    const p=item.player||{}; const fit=clamp(Math.round(Number(item.factors?.need||72))); const scarcity=clamp(Math.round(Number(item.factors?.scarcity||58)));
    const risk=String(p.risk||'Medium'); const safety=risk==='Low'?88:risk==='High'?58:74;
  ...
  function assignSlots(source,pool=[]){
    const available=[...source,...pool.filter(p=>!source.some(x=>x.name===p.name))]; const used=new Set();
    return SLOT_DEFS.map(([slot,pos])=>{
      const p=available.find(x=>!used.has(x.name)&&(pos==='ANY'||x.pos===pos||(pos==='FLEX'&&['RB','WR','TE'].includes(x.pos))));
      if(!p)return{slot,player:null}; used.add(p.name); return{slot,player:p};
    });
  }
  function odds(entries,projected=false){
    const filled=entries.filter(x=>x.player).length; const points=entries.reduce((s,x)=>s+Number(x.player?.proj||0),0);
  ...
  function depthCard(entry){
    ...
    return `<div class="tb-depth-card">${playerPhoto(p.name,'tb-depth-photo')}<div><small>${entry.slot}</small><strong>${esc(p.name)}</strong><span>${esc(p.team||'—')} · ${esc(p.pos||'—')}</span></div></div>`;
  }
  ...
  function supportCard(item,label){
    ...
    return `<button class="tb-support-card" data-explore="${encodeURIComponent(p.name)}">${playerPhoto(p.name,'tb-support-photo')}<div class="tb-support-copy"><small>${esc(label)}</small><strong>${esc(p.name)}</strong><span>${esc(p.team||'—')} · ${esc(p.pos)}</span><footer><i>${Math.round(p.proj||0)} pts</i><i>${m.survives}% live</i></footer></div><b class="tb-grade">${m.grade}</b></button>`;
  }
  function hero(item){
    const p=item.player,m=metrics(item);
    return `<article class="tb-hero"><div class="tb-hero-visual">${playerPhoto(p.name,'tb-hero-photo')}<span class="tb-position">${esc(p.pos)}</span></div><div class="tb-hero-content"><header><div><small>PLAYER UNDER CONSIDERATION</small><h2>${esc(p.name)}</h2><p>${esc(p.team||'—')} · ${esc(p.pos)}</p></div><div class="tb-hero-grade"><b>${m.grade}</b><small>GOOSE</small></div></header><div class="tb-goose-note"><small>GOOSE AI</small><p>Take ${esc(p.name)} now. Passing creates a ${100-m.survives}% chance he is gone before your next pick.</p></div><div class="tb-hero-stats"><span><small>PROJ</small><b>${Math.round(p.proj||0)}</b></span><span><small>FIT</small><b>${m.fit}</b></span><span><small>SCARCITY</small><b>${m.scarcity}</b></span><span><small>SURVIVES</small><b>${m.survives}%</b></span><span><small>RISK</small><b>${esc(m.risk)}</b></span><span><small>TIER</small><b>${m.tier}</b></span><span><small>CHAMP IMPACT</small><b>+${m.impact}%</b></span><span><small>NEED</small><b>${'★'.repeat(Math.max(1,Math.min(5,Math.round(m.fit/20))))}</b></span></div><div class="tb-hero-actions"><button data-take="${encodeURIComponent(p.name)}">Take Player</button><button class="secondary" data-report="${encodeURIComponent(p.name)}">Open Report</button></div></div></article>`;
  }
  function render(){
    const mount=document.getElementById('frontOfficeRoot'); const all=ranked(); if(!mount||!all.length)return false;
    const top=all.find(p=>p.name===selectedName())||all[0]; const main=itemFor(top); const others=all.filter(p=>p.name!==top.name);
    const currentEntries=assignSlots(roster()); const perfectEntries=assignSlots(roster(),[top,...others]); const currentOdds=odds(currentEntries); const perfectOdds=odds(perfectEntries,true);
    const counts=roster().reduce((a,p)=>(a[p.pos]=(a[p.pos]||0)+1,a),{}); const targets={QB:2,RB:2,WR:2,TE:1}; const priority=['QB','RB','WR','TE'].find(pos=>(counts[pos]||0)<targets[pos])||'WR';
    const pivots=others.slice(0,10).map(itemFor); const future=[...others.filter(p=>p.pos===priority),...others.filter(p=>p.pos!==priority)].slice(0,10).map(itemFor);
    mount.innerHTML=`<div class="tb-war-room"><header class="tb-roster-banner"><div class="tb-banner-head"><div class="tb-franchise"><img src="logo.png" alt="${esc(teamName())}"><div><small>YOUR FRANCHISE</small><h1>${esc(teamName())}</h1><p>Live roster · Mission ${(window.state?.drafted?.length||0)+1}</p></div></div>${metricBar(currentOdds)}</div><div class="tb-depth-line"><div class="tb-depth-title"><small>CURRENT TEAM</small><b>Built as you draft</b></div>${depthChart(currentEntries)}</div></header><div class="tb-live-ribbon"><b>GOOSE LIVE</b><span>${esc(currentOwner())} is selecting</span><span>${100-metrics(main).survives}% chance ${esc(top.name)} is gone by your next pick</span><span>${priority} is the next roster mission</span></div><section class="tb-decision-grid"><aside class="tb-side"><small>WHAT IF YOU PIVOT</small><h3>Pivot Paths</h3><div class="tb-support-grid">${pivots.map(x=>supportCard(x,'Alternate')).join('')}</div></aside><main class="tb-center">${hero(main)}<div class="tb-league-row"><span><small>LEAGUE LIVE</small><b>${esc(currentOwner())}</b></span><span><small>LIKELY POSITION</small><b>${priority}</b></span><button>Inspect Team</button></div></main><aside class="tb-side"><small>FUTURE BOARD</small><h3>Likely Available</h3><div class="tb-support-grid">${future.map(x=>supportCard(x,'Next pick')).join('')}</div></aside></section><footer class="tb-perfect-banner"><div class="tb-banner-head"><div class="tb-perfect-title"><small>PERFECT DRAFT</small><h2>Best Realistic Finished Roster</h2><p>Pre-draft prediction recalculates after every selection.</p></div>${metricBar(perfectOdds,true)}</div><div class="tb-depth-line"><div class="tb-depth-title"><small>PROJECTED TEAM</small><b>Best realistic path</b></div>${depthChart(perfectEntries)}</div></footer></div>`;
